...
namespace Player.Abilities
{
    ...
    public class IceWaveAbility : PlayerAbility
    {
        ...
        private void Awake()
        {
            Debug.Log(">>> IceWaveAbility Awake chamado! <<<");
        ...
        private void Start()
        {
            Debug.Log($"=== IceWaveAbility Start ===");
            ...
            if (wavePrefab != null)
            {
                Debug.Log($"✓ WaveEnergy carregado: {wavePrefab.name}");
            }
            else
            {
        ...
        {
            Debug.Log("=== Ice Wave Ativada! ===");
            
            if (wavePrefab == null)
            {
                Debug.LogError("❌ Ice Wave Prefab não atribuído!");
                return;
            }
            
            Debug.Log($"✓ Prefab encontrado: {wavePrefab.name}");
            ...
            {
                Debug.Log("⚠ Mana insuficiente para Ice Wave!");
                return;
            }
        ...
        private void CreateWaveObject(Vector3 position)
        {
            ...
            var damageComponent = obj.GetComponent<IceWaveDamage>();
            if (damageComponent == null)
            {
                damageComponent = obj.AddComponent<IceWaveDamage>();
            }
    ...
    public class IceWaveDamage : MonoBehaviour
    {
        ...
        private void OnTriggerEnter(Collider other)
        {
            if (other.CompareTag("monster") && !damagedMonsters.Contains(other.gameObject))
            {
                ...
                if (monster != null)
                {
                    ...
                    Debug.Log($"✓ Ice Wave causou {damage} de dano em {other.gameObject.name}");
                }
            }
        }
    }
